...
const Header = () => {
  ...
  return (
    <div className="py-3 bg-[#d70018] shadow-md">
      <div className="flex justify-between items-center container">
        ...
        <img
          className="w-[80px] aspect-auto filter brightness-0 invert"
        ...
        >
          <input
            className="w-full h-full text-black py-1 px-3 outline-none flex-1"
            placeholder="Tìm kiếm sản phẩm...."
            value={searchText}
            onChange={(e) => setSearchText(e.target.value)}
          />
          <div className="px-3 h-full bg-black flex items-center justify-center cursor-pointer">
        ...
        <div className="flex items-center relative">
          ...
          <Link to="/cart" className="relative flex items-center">
            <i className="text-[30px] text-white bx bx-cart-alt"></i>
            <p className="bg-[#df3346] text-xs w-[15px] h-[15px] text-white flex items-center justify-center rounded-full absolute top-[-6px] right-[0]">
              {cart.length}
            </p>
          </Link>
          {currentUser ? (
            <>
              <div className="flex items-center overflow-hidden ml-6">
                ...
                <button
                  ...
                  className="px-2 bg-[#df3346] text-white ml-3 flex items-center justify-center rounded-md"
                >
            ...
            <Link
              to="/auth/login"
              className="ml-6 py-1 px-3 bg-[#df3346] text-white rounded-md transition-colors"
            >
              Đăng nhập
            </Link>
          )}
